...
class Room
  attr_reader :alias, :desc, :name, :exits, :inside, :gateway, :library
  ...
  def to_s
    s = "      <object name=\"#{OBJECT_PREFIX}_#{@name}\">\n"
    s += "        <inherit name=\"gateway_type\" />\n" if @gateway
    s += "        <inherit name=\"editor_room\" />\n"
    s += "        <alias>#{@alias}</alias>\n"
    s += "        <description><![CDATA["
    s += @desc
    s += "]]></description>\n"
    s += "        <attr name=\"_initialise_\" type=\"script\">\n" if @gateway
    s += "          AddToHubGateway (this)\n" if @gateway
    s += "        </attr>\n" if @gateway
    s += "        <inside />\n" if @inside
    @objects.each do |k, v|
      s += "        <object name=\"#{OBJECT_PREFIX}_#{k.gsub(' ', '_').gsub(/[^\w]/, '').downcase}\">\n"
...
data = YAML.load_file("questLib.yml")
list = []
data.each do |h|
  if h.is_a? Hash
    # p h
...
File.open("#{OBJECT_PREFIX}Lib.xml", "w") do |file|
  flag = false
  list.each do |el|
    if el.is_a? Room
      file << el.to_s
      ...
      file << "  </object>\n" if flag
      file << "  <library>\n"
      file << "  <object name=\"#{OBJECT_PREFIX}_#{el.gsub(' ', '_').gsub(/[^\w]/, '').downcase}\">\n"
